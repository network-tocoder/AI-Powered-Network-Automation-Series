...
| 22 | [AWX MCP Integeration](#video-20-awx-claude-code-integration) | AWX + Ollama + Claude  | 🔜 Coming Soon |
...
We've built a complete GitOps pipeline across Videos 17-19: AWX running on K3s, syncing playbooks from GitHub, pulling dynamic inventory from NetBox. The "show version" playbook runs successfully against network devices. **Now we add an AI layer on top of AWX** — using Ollama (free, local LLM) connected to your existing AWX MCP server via an MCP client.

---

## 🏗️ Lab Topology (From Existing Series)
...
│                                                                              │
│   ┌───────────────────────┐    ┌───────────────────────┐                    │
│   │  Ansible/Automation   │    │      AWX Node         │                    │
│   │     Node              │    │    192.168.1.121       │                    │
│   │   192.168.1.119       │    │                        │                    │
│   │                       │    │  K3s + AWX             │                    │
│   │  - Ollama (NEW)       │    │  Port: 30052           │                    │
│   │  - MCP Client (NEW)   │    │  API: /api/v2/         │                    │
│   │  - AWX MCP Server     │    │                        │                    │
...
| Node | IP | Role | From Video |
|------|----|------|------------|
| EVE-NG Server | 192.168.1.100 | Hypervisor | Video 1 |
| Ansible Node | 192.168.1.119 | Automation + AI (this video) | Video 2 |
| NetBox | 192.168.1.120:8000 | Source of Truth | Video 7 |
| AWX Node | 192.168.1.121:30052 | Automation Platform (K3s) | Video 17 |
| vIOS-R1 | 192.168.1.201 | Network Device | Video 1 |
| vIOS-R2 | 192.168.1.202 | Network Device | Video 1 |
| vIOS-R3 | 192.168.1.203 | Network Device | Video 1 |
...
│                    │                     │                  │
│              ┌─────▼──────┐    ┌────────▼────────┐        │
│              │   Ollama   │    │  AWX MCP Server  │        │
│              │ qwen2.5:7b │    │  (your existing  │        │
│              │  (brain)   │    │   awx_mcp.py)    │        │
│              └────────────┘    └────────┬─────────┘        │
│                                         │ REST API         │
└─────────────────────────────────────────┼──────────────────┘
...
**Flow:** You type natural language → MCP Client sends to Ollama → Ollama decides which AWX tool to call → AWX MCP Server executes against AWX API → Results returned to you in plain English.
