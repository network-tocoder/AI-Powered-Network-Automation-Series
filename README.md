...
| 18 | [AWX Execution Environments](#video-18-awx-execution-environments) | Custom EE, ansible-builder | 🔜 Coming Soon |
...
| 19 | [AWX GitHub + NetBox](#video-19-awx-github--netbox-integration) | Git Sync, Dynamic Inventory | 🔜 Coming Soon |
| 20 | [AWX + Claude Code](#video-20-awx-claude-code-integration) | AI Agent + AWX API | 🔜 Coming Soon |
| 21 | [AWX MCP Server](#video-21-awx-mcp-server) | Custom MCP for AWX | 🔜 Coming Soon |
| 22 | [AWX + Gemini CLI](#video-22-awx-gemini-cli) | Free Remote AWX | 🔜 Coming Soon |
...
<details>
<summary>1. Prepare Rocky Linux Node in EVE-NG</summary>
...
helm install awx-operator awx-operator/awx-operator -n awx --wait

# Watch operator pod come up (takes 1-2 minutes)
kubectl get pods -n awx -w
# Wait until: awx-operator-controller-manager-xxxxx  2/2  Running

# Press Ctrl+C when Running, then verify
...
kubectl get svc -n awx
# Look for: awx-service  NodePort  ...  80:8052/TCP

# AWX URL
echo "AWX URL: http://$(hostname -I | awk '{print $1}'):8052"
# Example: http://192.168.1.121:8052

# Open in browser:
# URL: http://192.168.1.121:8052
# Username: admin
...
| UI accessible | Browser: `http://IP:8052` | Login page appears |
...
<details>
<summary>❌ Cannot access UI on port 8052</summary>
...
<details>
<summary>1. Install ansible-builder</summary>

```bash
# On your Ansible/AWX node
...
<details>
<summary>2. Create EE Project Directory</summary>
...
<details>
<summary>5. Create execution-environment.yml</summary>
...
<details>
<summary>6. Build the Execution Environment</summary>

```bash
# Build the EE image (takes 5-10 minutes)
...
# ansible-builder build --tag network-ee:1.0 --container-runtime docker --verbosity 3
...
#    - Image: network-ee:1.0  (local) or docker.io/username/network-ee:1.0
#    - Pull: Only pull if not present (for local) or Always pull (for registry)
# 4. Click Save

# Verify via kubectl
kubectl get pods -n awx
# New EE jobs will use this image
```
...
- [AWX Projects Documentation](https://ansible.readthedocs.io/projects/awx/en/latest/userguide/projects.html)
- [AWX Inventories Documentation](https://ansible.readthedocs.io/projects/awx/en/latest/userguide/inventories.html)
...
### v26.0 (2025-01-24)
- ✅ Video 17: AWX Installation on K3s - Complete step-by-step guide
...
- ✅ Video 18: AWX Execution Environments - Build custom EE
  - ansible-builder installation
  - requirements.yml (collections)
  - requirements.txt (Python packages)
  - EE build process
  - Push to registry (local + Docker Hub)
  - Add EE to AWX
- ✅ Video 19: AWX GitHub + NetBox Integration
  - GitHub project sync
  - Machine credentials (SSH)
  - NetBox credentials (API token)
  - Dynamic inventory from NetBox
  - Job template creation
  - GitOps workflow
- 🔄 Updated video index (renumbered AWX series)
